...
export default {
  ...
  methods: {
    ...
    setPopupMessage: function (course_id) {
      const popupInfoData = this.searchResult
        .map(subject => subject.courses.find(c => c.id === course_id))
        .filter(e => e)[0]
      if (popupInfoData) {
        const popupData = {
  ...
  computed: {
    weekOfDayData: function () {
      ...
      if (target) {
        ...
        resultCourses.courses = target.courses
          .map(c => {
            ...
            if (
              ...
              this.checkArr.dayofweek[0]
                .split(',')
                .includes(date.getDay() + 1 + '')
            ) {
              return c
            }
            return null
